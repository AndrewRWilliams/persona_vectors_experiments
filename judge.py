...
_JUDGE_RPM_LIMIT = int(os.getenv("JUDGE_REQUESTS_PER_MINUTE", "4800"))
_JUDGE_RATE_LIMITER = _AsyncRateLimiter(_JUDGE_RPM_LIMIT)


def set_judge_requests_per_minute(requests_per_minute: int):
    global _JUDGE_RPM_LIMIT, _JUDGE_RATE_LIMITER
    if requests_per_minute <= 0:
        raise ValueError("requests_per_minute must be positive")
    _JUDGE_RPM_LIMIT = requests_per_minute
    _JUDGE_RATE_LIMITER = _AsyncRateLimiter(_JUDGE_RPM_LIMIT)
...
class OpenAiJudge:
    ...
    async def logprob_probs(self, messages) -> dict:
        ...
        await _JUDGE_RATE_LIMITER.wait()
